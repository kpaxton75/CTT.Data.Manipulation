...
##
##        2. .csv file with a list of all active tags for the specified time period
##            -- Column names needed: TagId
##            -- other columns can also be in the file for your reference
##            -- Ensure all letters in TagId are capitalized
##
##        3. Raw Beep data files
...
########### Run function to get beep data and a count of the detections removed at different steps #############
    ...
    ## Output in R environment
        # beep.output - list containing beep_data (data frame with all beep data meeting specified time period and TagIds), count.date (number of rows removed that did not fall within specified time period),
                        # beep.bad.dates (data frame with beep data with bad dates, e.g. 1970), count.import (number of rows imported), count.NA (number of rows removed that had NA values - e.g. BaseStations have NA for NodeId),
                        # count.nodes (number of rows removed where the NodeId did not match the NodeId in the provided lookup table), count.duplicates (number of rows removed because of duplicate data, e.g. Base Stations with the same data),
                        # count.ghosts (number of rows removed where the TagId did not match a TagId in the provided lookup table), count.RadioId (number of rows removed that did not match the specified RadioId)
